...
using Wimi.BtlCore.Friendships.Cache;

namespace Wimi.BtlCore.Chat
{
    [AbpAuthorize]
    public class ChatMessageManager : BtlCoreDomainServiceBase, IChatMessageManager
    {
        ...
        private async Task HandleReceiverToSenderAsync(UserIdentifier senderIdentifier, UserIdentifier receiverIdentifier, string message, Guid sharedMessageId)
        {
            ...
            if (friendshipState == null)
            {
                ...
                var senderUser = await _userManager.GetUserByIdAsync(senderIdentifier.UserId);
                await _friendshipManager.CreateFriendshipAsync(
                    new Friendship(
                        receiverIdentifier,
                        senderIdentifier,
