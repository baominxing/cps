            ...
            this.chartOptions = {
                ...
                exporting: {
                    ...
                    buttons: {
                        contextButton: {
                            menuItems: [{
                                text: $.Translate("YieldAnalysis.HIGHCHARTS_COLUMN"),
                                onclick: function () {
                                ...
                                text: $.Translate("YieldAnalysis.HIGHCHARTS_LINE"),
                                onclick: function () {
                ...
                tooltip: {
                    formatter: function () {
                        return '<b>' + this.x + '</b><br/>' +
                            ...
                            $.Translate("YieldAnalysis.HIGHCHARTS_USE_TIME") + ': ' + this.series.userOptions.time[this.point.x] + $.Translate("YieldAnalysis.HIGHCHARTS_HOUR");
                    }
                },
                series: this.options.dataSource
            });
    ...
    $.widget("BZ.viewChart", {
        ...
        drawStatus: function (data) {
            ...
            for (var i = 0; i < length; i++) {
                ...
                var end_px = endtimes / this.resolution+1;//魔改
                ...
                if (isDraw) {
                    if (this.options.three) {
                        ...
                        st.push(self.paper.path(spath1.path1 + spath1.path2 + spath1.path3 + spath1.path4 + "z")
                                         .attr({
                                             "stroke-width": 0,
                                             "fill": "90-" + data[i].COLOR + "-#000:0-#000:0-" + data[i].COLOR//67.5-#dd4242-#ff4242:48-#A00:52-#A00  
                                         }).data("type", 1));
                    }
                    ...
                    if (this.options.three) {
                        ...
                        st.push(self.paper.path(spath2.path1 + spath2.path2 + spath2.path3 + spath2.path4 + "z")
                                         .attr({
                                             "stroke-width": 0,
                                             "fill": "0-" + data[i].COLOR + "-#000:0-#000:0-" + data[i].COLOR
                                         }).data("type", 3));
                    }
                    st.data("StartT", sdate);
                    st.data("EndT", edate);
                    st.data("StateName", data[i].NAME);
                    ...
                    if (this.options.url != null) {
                        ...
                        st.mouseover(function () {
                            ...
                            var message = ' <form class="form-horizontal" style="min-width: 260px;padding-top:0px;">' +
                                ...
                                '<label class="control-label callout-control-label">' + app.localize("StateName") + '：</label>' +
                                '<div class="controls controls-control-label">' +
                                '<span>' + this.data("StateName") + '</span>' +
                                '</div>' +
                                '</div>' +
                                '<div class="control-group" style="margin-bottom: 0px;">' +
                                '<label class="control-label callout-control-label">' + app.localize("StartTime") + '：</label>' +
                                '<div class="controls controls-control-label">' +
                                '<span>' + this.data("StartT").format("YYYY-MM-DD HH:mm:ss") + '</span>' +
                                '</div>' +
                                '</div>' +
                                '<div class="control-group" style="margin-bottom: 0px;">' +
                                '<label class="control-label callout-control-label">' + app.localize("EndTime") + '：</label>' +
                                '<div class="controls controls-control-label">' +
                                '<span>' + this.data("EndT").format("YYYY-MM-DD HH:mm:ss") + '</span>' +
                                '</div>' +
                                '</div>' +
                                '</form>';
                            var top, left;
...
;
                          ...
                          //  ajaxGet.abort();
                            ...
                            //$("#popover").myeditable({
                            //    title: '编辑<i id="BzEditerCancel" style="position: absolute; right: 10px;" class="fa fa-remove"></i>',
    ...
    $.widget("BZ.myeditable", {
        ...
        _create: function () {
            ...
            $(self.options.ok).click(function () {
                //提交
                var data = {
                    stateId: self.options.statusID,
                    memo: $("#BzEditerText").val()
                }
                //console.log(data);
                abp.services.app.statusDistributionMap.updateMemo(data).done(function (response) {
                    self.destroy();
                    self.element.popover("destroy");
                })
                // $.post("/api/services/app/statusDistributionMap/UpdateMemo",data), function (data) {
                //     if (data.Status == 0) {
...
(function ($, undefined) {
    $.widget("BZ.orderSurvey", {
        ...
        _create: function () {
            ...
            for (var a = 0; a < this.options.dataSource.Tasklist.length; a++) {
                if (a == 0) {
                    this.max_date = this.options.dataSource.Tasklist[0].END_DATE;
                }
                if (moment(this.max_date).isBefore(this.options.dataSource.Tasklist[a].END_DATE)) {
            ...
            for (var a = 0; a < this.options.dataSource.Tasklist.length; a++) {
                if (a == 0) {
                    this.min_date = this.options.dataSource.Tasklist[0].START_DATE;
                }
                if (moment(this.min_date).isBefore(this.options.dataSource.Tasklist[a].START_DATE)) {
        ...
        _drawbar: function () {
            ...
            for (var i = 0; i < self.options.dataSource.Tasklist.length; i++) {
                ...
                else if (i > 0 && i < self.options.dataSource.Tasklist.length - 1) {
                    ...
                    self.paper.rect(x, y, w, h).attr({
                        ...
                        .mouseover(function () {
                            ...
                            var tjson = {
                                ...
                                "投料量": mac.INPUT_NUM == null ? "-- --" : mac.INPUT_NUM,
                                "目标量": mac.TARGET_NUM == null ? "-- --" : mac.TARGET_NUM,
                                "产出": mac.YIELD,
                                "完成率": (mac.TARGET_NUM == null ? "-- --" : ((mac.YIELD / mac.TARGET_NUM) * 100).toFixed(1)) + "%",
                                "正品": mac.QUALIFIED_NUM == null ? "-- --" : mac.QUALIFIED_NUM,
                                "次品": mac.INFERIOR_NUM == null ? "-- --" : mac.INFERIOR_NUM,
                                "质量率": ((mac.QUALIFIED_NUM == null || mac.INFERIOR_NUM == null) ? "-- -- " : (mac.QUALIFIED_NUM / (mac.QUALIFIED_NUM + mac.INFERIOR_NUM) * 100).toFixed(1)) + "%",
                                "操作人员": mac.MEM_NAMES == null ? "-- --" : mac.MEM_NAMES
                            };
                            $("#display").jCallout({
                                message: self._template(tjson),
                                position: "top",
